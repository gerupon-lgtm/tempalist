...
保存データ直下の任意フィールド `supplySamplesAdded: true` は、管理向け4種のテンプレートと一括保存する端末内の追加済み状態です。未設定の旧データは追加対象です。通常の保存で保持し、サンプルを削除しても解除しません。業務データではないため共有・全体バックアップには含めず、移行先の追加済み状態を維持します。


## v0.6.0 消費期限通知の追加

管理項目に任意の消費期限、管理リスト単位の通知ON/OFF、設定画面の共通通知時刻を追加。保存形式3で期限を保持し、前日・当日の未来の通知を既存v2 APIで予約する。消費時は取消、購入時は期限をクリア、購入の取り消しは期限も復元する。従来の「管理には期限・通知なし」はv0.6.0で変更。共通の対応リスト自体の期限・あとキュー連携は追加しない。詳細と保存・バックアップ・タイムゾーン規約は [消費期限通知](消費期限通知.md) を参照。
