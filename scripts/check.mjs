...
for (const name of await readdir(new URL('../src/', import.meta.url),{recursive:true})) if (name.endsWith('.js')) execFileSync(process.execPath, ['--check', fileURLToPath(new URL('../src/' + name.replaceAll('\\','/'), import.meta.url))], {stdio:'inherit'});
