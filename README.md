...
v0.1.1は個人試用向けのフロント実装です。テンプレート管理、リストの作成・チェック・完了・再開、書き戻し、共有、バックアップ、保持期間と容量整理を使えます。ヘッダにアイコンと名称、フッターに版数とコピーライトを表示します。

通知、Service Workerによるオフライン起動、ホーム画面追加案内は後続P4です。配色は承認済みのA1改（白に近い背景・コーラル系アクセント）。候補からZen Kaku Gothic Newを仮採用し、取得失敗時は端末標準のゴシック系を使います。項目右端のつまみを約0.45秒長押しすると半透明カードで並べ替えでき、⋯メニューの「上へ／下へ」も使えます。公開・DNS・通知基盤の変更は行っていません。
