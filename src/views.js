...
export function settings(state,bytes,version) {
  return heading('設定とデータ','この端末のブラウザに保存しています。')+`<div class="settings-grid"><section class="settings-panel"><h2>完了リストの保持期間</h2><label for="retention">完了を確定した日から</label><select id="retention">${[['30d','30日'],['90d','90日'],['365d','1年（365日）'],['keep','削除しない']].map(([v,n])=>`<option value="${v}" ${v===state.settings.completedRetention?'selected':''}>${n}</option>`).join('')}</select><p>期間を過ぎた完了リストは、起動時に削除します。進行中のリストとテンプレートは対象外です。</p></section><section class="settings-panel"><h2>バックアップと取り込み</h2><p>ブラウザのデータを消すと、リストも失われます。大切なデータはファイルに保存してください。</p><div class="actions">${button('export','JSONを書き出す')}${button('import','JSONを取り込む')}</div><input class="file-input" id="import-file" type="file" accept=".json,application/json"></section><section class="settings-panel"><h2>保存容量の整理</h2><p>使用量の目安 ${(bytes/1024).toFixed(1)} KB<br>保存できる量はブラウザによって異なります。</p>${button('capacity','完了リストを整理する')}</section><section class="settings-panel"><h2>通知の接続確認</h2><p>通知基盤との接続を確認できます。通知の有効化は、基盤側の登録と連携の準備が整ってからご案内します。</p>${button('check-notification-connection','接続を確認する')}<p id="notification-connection-status" role="status" aria-live="polite">まだ確認していません。</p></section><section class="settings-panel"><h2>このアプリについて</h2><p>NOT EQUAL TEMPALIST<br>バージョン ${e(version)}<br>通知機能は準備中です。</p><p>テンプレートとチェック内容はサーバーに送信しません。</p></section></div>`;
}
