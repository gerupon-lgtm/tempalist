...
export function labelUnits(text){
...
export function detail(entity,kind) {
  ...
  return `<a class="back" href="#/${checklist?'lists':'templates'}">${icon('back',16)} 一覧に戻る</a>`+
    heading(entity.title??entity.name,checklist?'ひとつずつ確認していきましょう。':`${statusName[entity.status]} · チェックリストのひな型`,locked?'':button('edit-meta','編集','detail-edit'),'detail-heading')+
